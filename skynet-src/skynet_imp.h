...
struct skynet_config {
	...
	const char * logservice;        /* 日志服务名 (默认为 logger) */
};

/* 线程的类别, 作为线程初始化的参数, 它们的负值将被转为 unit32 整数并与服务句柄一样设置在线程特定数据中,
 * 因而需要它们的负值有别于服务的 handle */
#define THREAD_WORKER 0      /* 0 */
#define THREAD_MAIN 1        /* 0xFFFFFFFF */
#define THREAD_SOCKET 2      /* 0xFFFFFFFE */
#define THREAD_TIMER 3       /* 0xFFFFFFFD */
#define THREAD_MONITOR 4     /* 0xFFFFFFFC */
